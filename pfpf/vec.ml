...
let center_angle = 0.
let min_angle = Float.(center_angle - (normal_angle_range / 2.))
let max_angle = Float.(min_angle + normal_angle_range)
...
let collinear ~epsilon t1 t2 t3 =
  let open Float in
  let deviation =
    abs (((t2.y -. t1.y) *. (t3.x -. t2.x)) -. ((t2.x -. t1.x) *. (t3.y -. t2.y)))
  in
  deviation < epsilon
;;
...
let avg_point pts =
  scale (List.fold_left pts ~init:origin ~f:add) (1. /. Float.of_int (List.length pts))
;;
